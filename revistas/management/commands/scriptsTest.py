...
def salvar(titulo_artigo_br, titulo_artigo_en, autores, descricao_artigo_br, descricao_artigo_en, identifier_artigo, identifier_edicao, issn_revista, data_lancamento_edicao, palavras_chaves, revista_br, revista_en, nome_revista, sobre_edicao, edicao, nome_revista_en, sobre_edicao_en, edicao_en, link_pdf_artigo,
           identifier_categoria, nome_da_categoria):

    # Essa função salva as variaveis de entrada no banco de dados

    tem_artigo = Artigo.objects.filter(
        identifier__startswith=identifier_artigo)
    # print(tem_artigo)
    ...
    if tem_artigo:
        ...
        if tem_revista:
            ...
            if type(issn_revista) is str:
                if len(issn_revista) < 16:
                    nova_revista = Revista(
                        issn=issn_revista, nome_revista_portugues=nome_revista, nome_revista_english=nome_revista_en)
                    nova_revista.save()

        # Povoando tabela de edição
        tem_edicao = Edicao.objects.filter(
            identifier__startswith=identifier_edicao)
        ...
        if tem_edicao:
            ...
            edicao_sem_identifier = Edicao.objects.filter(
                edicao_portugues=edicao, revista=verificar_revista).first()
            ...
            if edicao_sem_identifier:
                ...
                if edicao_sem_identifier.revista_id == verificar_revista.id:
                    ...
                    nova_edicao = Edicao(edicao_portugues=edicao, edicao_english=edicao_en, data_lancamento=data_lancamento_edicao,
        ...
        tem_categoria = Categoria.objects.filter(
            identifier__startswith=identifier_categoria).first()

        if tem_categoria:
            pass
        else:
            revista_to_categoria = Revista.objects.get(issn=issn_revista)
            nova_categoria = Categoria(nome_categoria=nome_da_categoria,
                                       revista=revista_to_categoria,
                                       identifier=identifier_categoria)
        ...
        try:
            ...
            revista_to_except = Revista.objects.get(issn=issn_revista)
            edicao_to_artigo = Edicao.objects.filter(
                data_lancamento=data_lancamento_edicao, revista=revista_to_except).first()

        try:
            revista = Revista.objects.get(issn=issn_revista)
            # TODO: Testar se a categoria está certa
        ...
        novo_artigo = Artigo(titulo_portugues=titulo_artigo_br,
                             ...
                             descricao_portugues=descricao_artigo_br,
                             descricao_english=descricao_artigo_en,
                             identifier=identifier_artigo,
                             link_pdf=link_pdf_artigo,
                             edicao=edicao_to_artigo,
                             categoria=categoria_to_artigo)
        novo_artigo.save()

        # Povoando tabela de autores
        for a in range(len(autores)):

            tem_autor = Autor.objects.filter(nome_autor=autores[a])

            if tem_autor:

                autor_to_artigo = Autor.objects.get(nome_autor=autores[a])

                novo_artigo.autores.add(autor_to_artigo)
                continue

            novo_autor = Autor(nome_autor=autores[a])
            novo_autor.save()
        ...
        for s in range(len(palavras_chaves)):
            ...
            tem_palavra_chave = PalavraChave.objects.filter(
                assunto=palavras_chaves[s])

            if tem_palavra_chave:

                palavra_chave_to_artigo = PalavraChave.objects.get(
                    assunto=palavras_chaves[s])
                novo_artigo.palavras_chave.add(palavra_chave_to_artigo)
                continue

            nova_palavra = PalavraChave(assunto=palavras_chaves[s])
            nova_palavra.save()
            novo_artigo.palavras_chave.add(nova_palavra)


def revista_escolhida(oai_url_base):
    ...
    site_inicial = oai_url_base + '/oai?verb=ListRecords&metadataPrefix=oai_dc'
    proximo_site = oai_url_base + '/oai?verb=ListRecords&resumptionToken='
    site_categorias = oai_url_base + '/oai?verb=ListSets'
...
def run(revista_OAI):
    ...
    site_inicial, proximo_site, site_categorias = revista_escolhida(
        revista_OAI)
    site = site_inicial
...
#--------Recolhimento das informações das categorias--------#
    print("- Recolhimento das informações das categorias")
...
#--------Recolhimento das informações dos artigos-------#
    while True:
        ...
        for x in range(len(lista)):
            ...
            identifier_edicao = lista[x]['metadata']['oai_dc:dc']['dc:source'][len(
                lista[x]['metadata']['oai_dc:dc']['dc:source'])-1]
            issn = lista[x]['metadata']['oai_dc:dc']['dc:source'][len(
                lista[x]['metadata']['oai_dc:dc']['dc:source'])-2]

            if len(identifier_edicao) < 14:
                issn = identifier_edicao
                identifier_edicao_IfErro = lista[x]['metadata']['oai_dc:dc']['dc:identifier'][len(
                    lista[x]['metadata']['oai_dc:dc']['dc:identifier'])-1]
                identifier_edicao_IfErro = identifier_edicao_IfErro.split('p')[
                    0]
